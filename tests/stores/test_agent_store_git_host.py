...
def test_create_with_git_host_id_persists_it(store):
    ...
    agent = store.create(
        aid, "git-host-agent", f"{aid}/deadbeef",
        git_url="https://github.com/org/repo", git_ref="main",
        git_commit="abc123", git_host_id="host_aabbccdd",
    )
...
def test_update_with_git_host_id_sets_it_and_bumps_version(store):
    aid = generate_agent_id()
    store.create(
        aid, "git-host-agent2", f"{aid}/old",
        git_url="https://github.com/org/repo", git_ref="main",
        git_commit="old_sha", git_host_id="host_old",
    )
...
def test_update_without_git_host_id_preserves_existing(store):
    aid = generate_agent_id()
    store.create(
        aid, "git-host-agent3", f"{aid}/old",
        git_url="https://github.com/org/repo", git_commit="sha1",
